...
<template>
  <div class="flex flex-col mx-4 min-h-screen md:mx-10">
    <h1 class="text-lg pb-2 md:text-4xl">
      {{ playlistName }}
    </h1>
    <p class="pb-1 text-base">
      Add to the Request List & Upvote Your Favorites
    </p>
    <p v-if="!searchResults" class="text-sm py-2">
  ...
  mounted() {
    if (process.browser) {
      this.playlists = localStorage.getItem('playlists')
        ...
        : {}
      this.listObject = this.playlists[this.id]
      this.playlistName = this.listObject.name
      this.playlist = this.listObject.songs
    }
  },
  methods: {
    async handleSubmit() {
